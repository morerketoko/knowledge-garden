...
export function buildKnowledgeAskSystem(input: KnowledgeAskInput): string {
  const lines = [
    ...
    "2. 引用 Vault 来源时，path 必须真实存在（Vault 内）且来自本次检索/读取结果；禁止伪造、改写或猜测路径。",
    ...
    "",
    "硬规则：",
    "1. sources[].path 必须真实存在（Vault 内）且来自本次检索/读取结果；候选清单只是第一次检索结果，不是访问白名单——必要时可继续检索整个 Vault。",
    ...
    "",
    "检索边界说明：候选清单只是第一次本地检索的结果，不是访问白名单。系统可以继续执行 vault.search / vault.read / vault.list_folder 访问整个 Vault（范围由 Workbench Scope 决定）。你不得声称访问过未在候选/读取清单中出现的笔记，不得伪造路径；需要更多材料时，在 followUps 中建议继续检索的方向。",
    "",
